...
// Fetch products
$sql = "SELECT product_id, product_name, price, roll_price, category, status FROM products";
...
.bg-success { 
 ...
 .bg-warning { 
  ...
  <nav class="navbar navbar-expand-lg">
        ...
        <div class="col-10 p-4">
          ...
          <!--div class="dropdown">
            ...
            <button
              ...
              onclick="window.location.href='add_product.php'" style="margin-right: 100px;">
              + ADD NEW
            </button>
          </div>
          ...
          <div class="card mt-4" style="max-height: 700px; width: 1400px; overflow: auto; ">
            <div class="card-body">
              <h5 class="card-title">Available Products
              <a href="color_list.php" style="margin-left: 1000px; color:#d9b65d">
                Product Colors List
              </a>
            </h5>
          </div>
    ...
    <tbody>
        <?php
        // Assuming $result is the result from a database query
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Assuming $status and $statusClass are assigned correctly
                ...
                echo "<tr>
                    ...
                    <td>₱" . number_format($row['roll_price'], 2) . "</td> 
                    <td>
...
function filterProducts() {
    ...
    products.forEach((product) => {
        ...
        let productName = product.querySelector('td:nth-child(1)').textContent.toLowerCase();
        let productId = product.querySelector('td:nth-child(2)').textContent.toLowerCase();

        // Check if input matches product name or product ID
        if (productName.includes(input) || productId.includes(input)) {
            product.style.display = ''; // Show row
            product.style.backgroundColor = 'yellow'; // Highlight matching row
        } else {
            product.style.display = 'none'; // Hide row
            product.style.backgroundColor = ''; // Remove highlight
        }
    });
}
