...
if (isset($_POST['login'])) {
    ...
    // Check if the user exists in `users_credentials`
    ...
    if ($result->num_rows > 0) {
        ...
        $lockout_time = isset($row['lockout_time']) ? new DateTime($row['lockout_time']) : null;

        if ($lockout_time && $current_time < $lockout_time) {
            $remaining_time = $current_time->diff($lockout_time);
            $message = "Account locked. Try again in " . $remaining_time->i . " minute(s).";
        } else {
            if ($password1 === $row['PASSWORD']) {
                // Reset login attempts and lockout time
                $stmt = $conn->prepare("UPDATE users_credentials SET login_attempts = 0, lockout_time = NULL WHERE EMAIL = ?");
                $stmt->bind_param("s", $email);
                $stmt->execute();
                ...
                if ($attempts >= 3) {
                    $lockout_end = new DateTime();
                    $lockout_end->modify('+3 minutes');
                    $lockout_end_str = $lockout_end->format('Y-m-d H:i:s');

                    $stmt = $conn->prepare("UPDATE users_credentials SET login_attempts = ?, lockout_time = ? WHERE EMAIL = ?");
                    $stmt->bind_param("iss", $attempts, $lockout_end_str, $email);
                    $stmt->execute();

                    $message = "Too many failed attempts. Account locked for 3 minutes.";
                } else {
                    $stmt = $conn->prepare("UPDATE users_credentials SET login_attempts = ? WHERE EMAIL = ?");
                    $stmt->bind_param("is", $attempts, $email);
                    $stmt->execute();

                    $message = "Invalid password. You have " . (3 - $attempts) . " attempt(s) left.";
                }
            }
        }
    } else {
