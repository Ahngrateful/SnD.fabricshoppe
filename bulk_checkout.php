...
$stmt = $pdo->prepare("
    SELECT bsc.product_id, bsc.bulk_cart_id, bsc.product, bsc.unit_price, bsc.customer_id, bsc.roll_price, bsc.rolls, bsc.yards, bsc.color, bsc.color_id, bsc.item_subtotal, bsc.delivery_method, bsc.delivery_date, bsc.payment_method,
    pc.color_id
    FROM bulk_shopping_cart bsc
    JOIN product_colors pc ON bsc.color_id = pc.color_id
    WHERE bsc.customer_id = :user_id
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cod_place_order'])) {
    try {
       ...
       //save cod_payment
       $stmt = $pdo->prepare("
       INSERT INTO cod_payment (customer_id, method) 
       VALUES (:customer_id, :method)
   ");
   $stmt->execute([
       ':customer_id' => $product['customer_id'],
       ':method' => $product['payment_method'],
   ]);
