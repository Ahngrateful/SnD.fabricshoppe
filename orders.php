...
$sql = "SELECT order_num, customer_id, total_price, delivery_option, payment, status, order_date, track_num FROM order_details";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td><a href='order_details.php?order_num=" . urlencode($row['order_num']) . "'>" . htmlspecialchars($row['order_num']) . "</a></td>";
        ...
        echo "<td>" . htmlspecialchars($row['track_num']) . "</td>";
...
?>
        ...
        <button id="bulk_orders" class="btn btn-warning" onclick="window.location.href='bulkorders_list.php';">
            Check bulk orders
        </button>
    </div>
        </div>
